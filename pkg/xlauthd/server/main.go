...
func (xl *XlauthServerMain) GetHandler() ldap.Handler {
	...
	if xl.GroupHtgroup != "" {
		_stat, err := os.Stat(xl.GroupHtgroup)
		if err == nil {
			if _stat.ModTime().Unix() > xl.GroupStamp {

				xl.GroupStamp = _stat.ModTime().Unix()
				xl.GroupAccounts, err = xcrypt.ReadFromHtgroup(xl.GroupHtgroup)
				if err != nil {
					xl.LogErr(nil, "read htgroup failed: %v", err)
					return routes
				}
			}
		}
	...
	routes.Search(xl.HandleSearchGroupBase).BaseDn(GroupOU + "," + xl.BaseDN)
